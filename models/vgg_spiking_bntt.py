    ...
    def forward(self, inp):
        ...
        for t in range(self.timesteps):
            ...
            for i in range(len(self.conv_list)):
                ...
                rst = torch.zeros_like(mem_conv_list[i])
...
class SNN_VGG11_BNTT(nn.Module):
    ...
    def forward(self, inp):
        ...
        mem_conv6 = torch.zeros(batch_size, 512, self.img_size // 8, self.img_size // 8, device=device)
        mem_conv7 = torch.zeros(batch_size, 512, self.img_size // 16, self.img_size // 16, device=device)
        mem_conv8 = torch.zeros(batch_size, 512, self.img_size // 16, self.img_size // 16, device=device)
